-- Aplicado pelo pipeline "DB migrations -> Azure" (ver db/azure-migrations/README.md),
-- que e o caminho documentado no CLAUDE.md para mudancas de estrutura, politicas RLS
-- incluidas. Sem begin/commit: o pipeline ja corre com --single-transaction, e um
-- bloco aqui dentro aninharia. Rastreado em public._xpto_migrations.
--
-- As migracoes 20 a 26 desta Central foram aplicadas directamente na VM e ficam em
-- db/ como registo. A partir daqui, tudo passa por aqui: e auditavel, e idempotente,
-- e nao depende de ninguem ter acesso de linha de comando a maquina da base.
